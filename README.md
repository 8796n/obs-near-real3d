...
- **立体グレーディング(S字, 任意)**: `e = d - convergence` に奇関数の `tanh` S字を掛けて視差を非線形化(`disp = strength * lerp(e, tanh(K·e)/tanh(K), grading)`)。中距離を濃く・近景/遠景の最大視差を丸めるので、**同じ中距離の奥行きを低いピーク視差で**得られ、最前景シルエットの裂け(後述のラバーシート)が増えにくい。単調かつ `e=0` で 0 なので**深度順序と収束面は保たれる**。既定 0(=直線)。
...
- 代償として、**前景の陰に隠れていた背景(disocclusion)は黒穴でなく引き伸ばし**になる。急な深度段差(前景シルエット vs 遠景)で視差を強くすると輪郭が裂ける——DIBR 特有の "rubber-sheet" アーティファクト。**エッジ平滑**(深度エッジの Gaussian 平滑)で深度段差をなだらかにして緩和する(mesh-warp 相当の近似)。視差量に比例して悪化するので、効果は控えめが基本。
...
3. フィルタ設定は **3D / 安定化 / デバッグ** の3グループ（各項目はホバーで詳細ツールチップ）。以下は UI 表示どおりの日本語ラベル。
   ...
   **3D**
   ...
   - **立体グレーディング（S字）**（0–1、既定 0。深度→視差の対応を直線から `tanh` の S字へブレンドし、中距離の立体感を強めつつ近景/遠景の最大視差を丸める。`3D強度`を上げるのと違い、近景の視差(と輪郭破綻)を上げずに中距離の奥行きを濃くできる。深度の順序と収束面は不変。**0=直線(従来どおり)**）
   - **左右反転**（立体が反転して見えるとき）
...
- **v0.5.0**
  - **追加(3D)**: **立体グレーディング(S字)** スライダ(0–1、既定 0)。深度→視差を直線から `tanh` の S字へブレンドし、中距離の立体感を強めつつ近景/遠景の最大視差を丸める。**同じ中距離の奥行きをより低いピーク視差で**得られるため、最前景シルエットの裂け(DIBR ラバーシート)が増えにくい。単調・`e=0` で 0 ゆえ深度順序と収束面は不変。0=直線(従来挙動)。
