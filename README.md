...
- **時間安定化(flicker 対策)**: 単眼深度モデルはフレーム独立推論なので、画面のどこかが動くと**静止領域の深度まで毎フレーム揺れる**(breathing。MiDaS/DPT/DA V2 共通の限界)。対策として **自前実装の軽量オプティカルフロー(ピラミッド型 Lucas-Kanade、OpenCV 非依存)で前フレーム深度を動き補償ワープ → 残差に応じて適応合成**し、加えて正規化レンジを EMA 平滑。シーンカットはフレーム間平均差分で検出してスナップ追従する。比較用モードでは、現在深度の局所範囲へ過去深度を制限する **history clipping** と、高速移動・フロー失敗付近の履歴を捨てる **reactive mask** も試せる(`flow_stabilizer.hpp`)。
...
3. フィルタ設定(**3D / 安定化 / デバッグ** の3グループ。各項目はホバーで詳細ツールチップ):
   ...
   - **Stabilize depth** / **Stabilization strength**(時間平滑化、既定 on / 0.4。上げるほど安定・動きは僅かに遅延・残像寄り)
   - **Temporal mode (comparison)**(ローカル比較用、既定 `Legacy`。`Legacy`=従来式 / `History clipping`=現在深度の局所範囲へ履歴を制限 / `Reactive mask + clipping`=高速移動・フロー失敗付近では履歴を破棄)
