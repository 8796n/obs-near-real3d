...
- **時間安定化(flicker 対策)**: 単眼深度モデルはフレーム独立推論なので、画面のどこかが動くと**静止領域の深度まで毎フレーム揺れる**(breathing。MiDaS/DPT/DA V2 共通の限界)。対策として **自前実装の軽量オプティカルフロー(ピラミッド型 Lucas-Kanade、OpenCV 非依存)で前フレーム深度を動き補償ワープ → 残差に応じて適応合成**し、加えて正規化レンジを EMA 平滑。シーンカットはフレーム間平均差分で検出してスナップ追従する。深度マップは低周波で、フロー誤差は残差フォールバックが吸収するため近似フローで十分(`flow_stabilizer.hpp`)。
