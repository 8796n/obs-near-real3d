...
> **通常版 / 軽量版（`-lite`）**: 各リリースには既定の **448×252** モデル同梱版と、**224×126** モデル同梱版（`…-lite-installer.exe` / `…-lite-…zip`）の2種類があります。`-lite` は推論トークンが 1/4 で**おおむね 2〜3 倍速**になり、**AMD などの非力な iGPU** で 3D 化が重いときに有効（深度の細部はやや粗くなります）。プラグイン本体は共通で、**同梱する深度モデルだけが違う**ので、両方を入れる必要はなく片方を入れれば置き換わります。どちらが動作中かは OBS のログ `[near-real3d] depth model loaded (WxH)` で確認できます。
...
3. フィルタ設定(**3D / 安定化 / デバッグ** の3グループ。各項目はホバーで詳細ツールチップ):
   ...
   - **Frame-matched depth(深度と映像を一致)**(既定 off。映像表示を実測の深度レイテンシ(~0.1s)ぶん遅らせ、表示フレームに**世代＋撮影タイムスタンプで対応する深度**を当てて動き中の“深度が遅れる”ズレを解消し、**ハードカットを跨いでも 3D を維持**する。チェック付きグループで、下記**遅延マージン**を内包(本トグル off 時はマージンは無効化)。**追加 VRAM＋出力遅延**と引き換え。音声は自動で同じだけ遅延し A/V を保つ。ライブ配信では遅延は配信側バッファに吸収されやすい)
     - **Delay margin(遅延マージン)**(0–4、既定 2。**深度と映像を一致** が on のときだけ有効。実測レイテンシに上乗せする追加遅延フレーム数。カット直後の新シーン深度を映像より先に届かせ、カット時の一瞬の 2D を防ぐ。0=レイテンシちょうど(たまに 1 フレームだけ 2D が出る)。上げるほど確実だが出力遅延が増える)
   - **Show depth map**(デバッグ: **左=深度マップ / 右=元画像**を並べて表示。深度と原画を見比べられ、静止スキップ中は深度が固定・原画のみ更新されるので、静止判定しきい値の効きを目視確認しやすい)
   - **Tint by depth-match state(深度マッチ状態で着色)**(デバッグ: 表示フレームに合うキャッシュ深度が無い=2D フォールバック中は**赤**、視差ランプ復帰中は**黄**、フル 3D は無着色。ハードカット直後の挙動を一目で確認できる。検出カットと 2D 継続 ms を OBS ログにも出力。ONNX 深度の稼働中のみ有効)
   - **Disable depth cache (1 slot, A/B)(深度キャッシュ無効・1枚)**(デバッグ A/B: 深度キャッシュを実効 1 枚に制限し、キャッシュ導入前の「最新深度のみ」挙動を再現。遅延モードで本項 on/off を切り替えると、カット跨ぎの 3D 維持の効きを同じ素材で比較できる)
...
| **軽量版** `-lite` | 224×126 | 144 | **2.4 ms** | **3.1x 速** |

トークンは 1/4 だが速度は 3.1x（入出力転送や LayerNorm 等の固定費が小サイズで相対的に効くため 4x には届かない）。深度の精細さは落ちるが、深度は低周波情報なので立体感への影響は小さい。**非力な iGPU ほど重い推論を削る効果が大きい**ため、AMD iGPU 等では軽量版が実用差を生む。（参考: 16:9 化で正方 392²(10.8 ms)より通常版が ~0.71x 軽い＝576 vs 784 トークン。アスペクトも無歪み）
...
|---|---|---|---|---|
| FP32 | 20.0 ms | 99MB | — | |
| **FP16** | **10.8 ms** | 50MB | 0.012 | ~1.85x 速・無劣化・FP32 I/O ドロップイン（**既定**。実寸は通常版 448×252=7.5ms / 軽量版 224×126=2.4ms） |
...
**深度ワーカーは「ONNX 推論 → 時間安定化」を直列処理**する点に注意。RTX 3060 実測で時間安定化（自前オプティカルフロー）は約 **28ms**、安定化全体で約 **35ms**。ONNX(通常版 448×252 FP16 ~7.5ms)と合わせると **1 深度フレーム ≈ 43ms** になり、**時間安定化 ON では実効の深度更新は ~20fps が上限**（それ以上を選んでもワーカーが追いつかず一部フレームは間引かれる）。**30/60fps を活かすなら時間安定化を OFF**（ONNX のみ ~8ms）にするか、より高速な GPU/CPU が要る。軽量版(224×126)なら ONNX が ~2.4ms、時間安定化も画素数比でおおむね 1/4（概算 ~7ms）に下がるため、この上限がかなり上がる。warp（表示）は常に毎フレーム。

軽くしたいとき: **軽量版（`-lite`）インストーラを使う**（224×126 モデル同梱、上記参照）/ **Stop inference when static**(既定 on)/ **Depth update rate を下げる** / **時間安定化を OFF** / 入力ソース解像度を下げる。別寸が要れば `tools/export_onnx.py --width … --height …` で再エクスポートし、その `.onnx` を同梱するだけ（プラグインがモデルから寸法を読むのでコード変更は不要）。
...
- 深度はソース/各眼と同じ **16:9(448×252、`-lite` は 224×126)** で推論するためアスペクト無歪み（旧 392² 正方の横圧縮を解消）。**推論寸は同梱モデルの入力形状から自動取得**（`OrtDepth::Init`）。別寸へは `tools/export_onnx.py` で再エクスポートして差し替えるだけ（モデル無し時のフォールバック寸は `DEFAULT_INFER_W/H`）。
...
- **v0.4.0**
  - **追加**: **軽量版モデル(224×126)** と、**通常版 / 軽量版(`-lite`) の 2 インストーラ配布**。重みは同じ DA V2 small（≈50MB）で入力解像度＝計算量だけを下げる。RTX 3060 / DirectML / FP16 実測で ONNX 1 回が **7.5ms → 2.4ms（3.1x 速、ViT トークン 576→144）**。**AMD 等の非力な iGPU** で 3D 化が重いとき向け（深度の精細さはやや粗くなる）。プラグイン本体(DLL)は共通で、同梱する `.onnx` が違うだけ。
  ...
  - **追加(UI)**: フィルタ設定画面トップの識別行に、**動作中モデルの寸法**を表示（例 `[model 224x126]`）。ログを見なくても通常版/軽量版を判別できる。
...
1. **一度だけ準備**: 深度モデルを安定した URL（GitHub Release アセット等）に置き、リポジトリの **Actions 変数**（Settings → Secrets and variables → Actions → Variables）に設定。モデルは巨大なので Git には含めない。
   ...
   - `MODEL_LITE_URL` / `MODEL_LITE_SHA256` …（任意）軽量版 **224×126** モデルの直リンクと SHA-256。`MODEL_LITE_URL` を設定すると CI が `-lite` 成果物（`…-lite-installer.exe` / `…-lite-…zip`）も生成・添付する。未設定なら `-lite` はスキップ（タグリリースで `MODEL_LITE_URL` を設定したときは `MODEL_LITE_SHA256` も必須）。
...
3. **ローカルでも生成可**: `setup.ps1` → `build.ps1` → `package.ps1 -Version 0.3.1` で `dist/` に ZIP（＋Inno Setup があればインストーラ）。軽量版は同じ DLL のまま `package.ps1 -Version 0.3.1 -Variant lite`（要 `models/dav2_224x126.onnx`）。初回や CI 整備前の手動アップロードに。
