...
3. フィルタ設定は **3D / 安定化 / デバッグ** の3グループ（各項目はホバーで詳細ツールチップ）。以下は UI 表示どおりの日本語ラベル。
   ...
   **3D**
   ...
   - **フラット保護**（0–1、既定 0。ソースのテクスチャが乏しい領域=黒/白レターボックス帯・平坦 UI パネル・なめらかな空などで**視差をゼロへ弱める**。そこは単眼深度が深度を捏造するだけで、warp すると枠/UI の**直線がせん断**される。これらを画面面に貼り付けて真っ直ぐに保ち、テクスチャのある本編はフル 3D のまま。**画像ベースなのでフレーム内部の帯/マージンにも効く**（外周限定でない）。上げると平坦な空・壁の立体感も失われる。**主にスクリーンキャプチャ/レターボックス素材向け**。0=オフ）
   - **左右反転**（立体が反転して見えるとき）
...
- **v0.5.1**
  - **追加(3D)**: **フラット保護** スライダ(0–1、既定 0)。テクスチャの乏しい領域(黒/白レターボックス帯・平坦UIパネル・なめらかな空)で視差をゼロへ弱め、単眼深度が捏造した平坦領域の直線せん断(枠/UIの歪み)を抑える。画像ベースでフレーム内部の帯/マージンにも効く(外周限定でない)。主にスクリーンキャプチャ/レターボックス素材向け。
